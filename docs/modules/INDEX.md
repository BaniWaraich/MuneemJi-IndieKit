...
| ID                                           | Module                           | Status      | One-liner                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------- | -------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [D01](./D01-bank-statement-upload.md)        | bank-statement-upload            | IMPLEMENTED | BO uploads statement → metadata row → `muneem/statement.uploaded` Inngest event → status surface; the user-facing on-ramp for D02.                                                                                                                                                                                            |
| [D02](./D02-statement-format-extraction.md)  | statement-format-extraction      | IMPLEMENTED | Any statement (PDF or CSV) → normalised Markdown KV. PDF: pdfplumber classifies pages (text / scanned / blank) → GPT-4o mini on text, GPT-4o vision on scans → balance validation. CSV: GPT-4o mini straight-through. Bank identification deferred (`bank_identifier` always null). **No business logic.**                    |
...
| [D04](./D04-invoice-submission.md)           | invoice-submission               | SPECCED     | BO or CA uploads invoice/receipt files; pre-sign → store → `muneem/document.uploaded`. Storage + metadata only (no OCR/match).                                                                                                                                                                                                |
...
| [D06](./D06-transaction-invoice-matching.md) | transaction-invoice-matching     | SPECCED     | Heuristic linking of documents (F10/D04) to `needs_invoice` transactions; FX-aware amount tiers; sole writer of `transaction_document_matches` + `match_status` transitions.                                                                                                                                                  |
