...
- On success: emits one JSON object to stdout and exits 0:
    { "pages": [ { "page": N, "text": "..." } ] }
- If the PDF is encrypted and NO password was supplied: emits
    { "encrypted": true } to stdout and exits 2.
- If a password WAS supplied but it is wrong: emits
...
# correct password raises pdfminer.pdfdocument.PDFPasswordIncorrect. pdfplumber
# raises the SAME exception whether no password or a wrong password was given —
# the two cases are distinguished here by whether a password arg was supplied.
