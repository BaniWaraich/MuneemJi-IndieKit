...
export default async function ClientDetailPage({
  ...
  const [client, contacts, statements, profile] = await Promise.all([
    db.query.clientOrgs.findFirst({
      where: and(
        eq(clientOrgs.id, id),
        eq(clientOrgs.firmId, session.user.firmId),
      ),
    }),
    db.select().from(clientContacts).where(eq(clientContacts.clientOrgId, id)),
    db
      .select({
        ...
        <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-800">
              Complete client setup before uploading statements
            </p>
            <p className="mt-0.5 text-xs text-amber-700">
              Statement parsing requires a business profile. Without it, D03
              will fail.
            </p>
          </div>
          <Link
