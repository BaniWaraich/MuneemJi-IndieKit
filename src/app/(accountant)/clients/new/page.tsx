...
export default function NewClientPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!res.ok) {
      const zf = data?.details?.fieldErrors as
    ...
    router.push(`/clients/${data.client.id}/onboarding`);
  };
        ...
        <div className="mt-6 space-y-4 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          <div>
            ...
            <select
              ...
              value={country}
              onChange={(e) =>
                setCountry(e.target.value as keyof typeof COUNTRY_DEFAULTS)
              }
              className={input()}
            >
            ...
            <p className="mt-1 text-xs text-neutral-500">
              Currency ({currency}) and tax regime ({taxLabel}) are set
              automatically.
            </p>
          </div>
          ...
          <div>
            ...
            />
            {errors.taxNumber && (
              <p className="mt-1 text-xs text-red-600">{errors.taxNumber}</p>
            )}
          </div>
